...
OLED_Handle* OLED_Init(I2C_HandleTypeDef hi2c) {
    ...
    holed->hi2c = &hi2c;
    Debug_Printf("HI2C", __FUNCTION__, __LINE__);
...
static HAL_StatusTypeDef OLED_Area_Refresh(OLED_Handle* const holed, uint8_t l, uint8_t r) {
...
HAL_StatusTypeDef OLED_Refresh(OLED_Handle* const holed) {
    ...
    while (l <= OLED_WIDTH) {
        ...
        if (l <= OLED_WIDTH) OLED_Area_Refresh(holed, l, r);
 ...
 */
HAL_StatusTypeDef OLED_SendCommand(OLED_Handle* const holed, uint8_t data) {
    return HAL_I2C_Mem_Write(holed->hi2c, OLED_I2C_ADDRESS, OLED_COMMAND_ADDRESS, I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
}
 ...
 */
HAL_StatusTypeDef OLED_SendData(OLED_Handle* const holed, uint8_t data) {
    return HAL_I2C_Mem_Write(holed->hi2c, OLED_I2C_ADDRESS, OLED_DATA_ADDRESS, I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
}
...
HAL_StatusTypeDef OLED_BOOL_DrawChar(OLED_Handle* const holed, uint8_t x, uint8_t y, uint8_t c, uint8_t bool) {
    return OLED_BOOL_DrawBMP(holed, x, y, 6, 8, ((uint8_t *)ASCII) + 6 * (c - 32), bool);
}

HAL_StatusTypeDef OLED_DrawChar(OLED_Handle* const holed, uint8_t x, uint8_t y, uint8_t c) {
    return OLED_BOOL_DrawBMP(holed, x, y, 6, 8, ((uint8_t *)ASCII) + 6 * (c - 32), OLED_BOOL_Replace);
}
